---
title: "C++ OOP MCQs"
description: "Top multiple-choice questions (MCQs) on C++ Classes and Objects for interview preparation. Covers constructors, access specifiers, friend functions, and object creation."
keywords:
  - C++
  - C++ MCQs
  - C++ Questions
  - C++ Programming
  - C++ Interview Questions
  - C++ Quiz
  - C++ Multiple Choice Questions
  - C++ Objective Questions
  - C++ Basics
  - C++ Concepts
  - C++ Fundamentals
  - C++ Exam Questions
  - C++ Test Questions
  - C++ Online Test
  - C++ Practice Questions
  - Learn C++
  - C++ Data Structures
  - C++ STL
  - Object-Oriented Programming
  - OOP in C++
  - C++ Functions
  - C++ Classes
  - C++ Objects
  - C++ Constructors
  - C++ Destructors
  - C++ Access Specifiers
  - C++ Scope Resolution Operator
  - C++ Friend Function
  - C++ Inheritance
  - C++ Polymorphism
  - C++ Abstraction
  - C++ Encapsulation
  - Advanced C++ Topics
  - Multi-threading in C++
  - Competitive Programming in C++
  - C++ Coding Interview Questions

tags:
  - C++
  - C++ MCQs
  - C++ Interview Questions
  - C++ Online Test
  - C++ Quiz
  - C++ Objective Questions
  - C++ OOP
  - C++ Classes
  - C++ Objects
  - C++ Functions
  - C++ Programming Questions
  - Learn C++
  - C++ Test
  - C++ Beginner Guide
  - C++ Coding Questions
  - C++ Technical Interview
  - C++ Coding Interview
  - OOP in C++
  - C++ Constructors
  - C++ Destructors
  - C++ Polymorphism
  - C++ Virtual Functions
  - C++ Competitive Programming
  - Data Structures in C++
  - Algorithms in C++
  - STL in C++
  - C++ Performance Optimization
---

import AdBanner from '@site/src/components/AdBanner';
import { Question } from '../../Question';

# Object-Oriented Programming (OOP)

<Question
  question="1).What does OOP stand for?"
  options={['Organized Object Programming', 'Object-Oriented Procedure','Object-Oriented Programming', 'None of the above']}
  answer="Object-Oriented Programming"
/>
<Question
  question="2).Which of the following is not a feature of OOP?"
  options={['Compilation', 'Inheritance', 'Polymorphism', 'Encapsulation']}
  answer="Compilation"
/>
<Question
  question="3).Which keyword is used to define a class in C++?"
  options={['object', 'struct', 'class' , 'None of the above']}
  answer="class"
/>
<Question
  question="4).What is an object in C++?"
  options={['A type of function', 'A variable', 'An instance of a class' , 'None of the above']}
  answer="An instance of a class"
/>
<Question
  question="5).What is a key benefit of OOP?"
  options={['Faster compilation', 'Code reuse', 'Reduced memory usage' , 'None of the above']}
  answer="Code reuse"
/>
<Question
  question="6).Which feature of OOP ensures code security?"
  options={['Inheritance', 'Encapsulation', 'Abstraction', 'Polymorphism']}
  answer="Encapsulation"
/>
<Question
  question="7).What is the process of creating objects from a class called?"
  options={['Compilation', 'Execution', 'Instantiation', 'Initialization']}
  answer="Instantiation"
/>
<Question
  question="8).Which feature of OOP allows a class to acquire properties of another class?"
  options={['Encapsulation', 'Polymorphism', 'Abstraction', 'Inheritance']}
  answer="Inheritance"
/>
<Question
  question="9).What is an abstract class in C++?"
  options={['A class with no methods', 'A class with at least one pure virtual function', 'None of the above', 'A class that cannot have objects']}
  answer="A class with at least one pure virtual function"
/>
<Question
  question="10).What is a virtual function in C++?"
  options={['A function that is always public', 'None of the above', 'A function that is static', 'A function that can be overridden in a derived class']}
  answer="A function that can be overridden in a derived class"
/>

<div>
<AdBanner />
</div>

<Question
  question="11).Which of the following best defines encapsulation?"
  options={['Only defining variables in a class', 'None of the above', 'Binding data and functions together', 'Hiding implementation details']}
  answer="Binding data and functions together"
/>
<Question
  question="12).Which type of function defines polymorphism?"
  options={['Virtual function', 'Inline function', 'Static function', 'Friend function']}
  answer="Virtual function"
/>
<Question
  question="13).Which OOP concept allows multiple functions with the same name but different parameters?"
  options={['Function Overloading', 'Abstraction', 'Inheritance', 'Encapsulation']}
  answer="Function Overloading"
/>
<Question
  question="14).What is the main purpose of a constructor?"
  options={['None of the above', 'To initialize an object', 'To destroy an object', 'To copy an object']}
  answer="To initialize an object"
/>
<Question
  question="15).Which constructor is used to create a copy of an existing object?"
  options={['Copy constructor', 'Virtual constructor', 'Default constructor', 'Parameterized constructor']}
  answer="Copy constructor"
/>
<Question
  question="16).Which of the following is not a type of constructor in C++?"
  options={['Virtual constructor', 'Default constructor', 'Copy constructor', 'Parameterized constructor']}
  answer="Virtual constructor"
/>
<Question
  question="17).Which of the following cannot be inherited?"
  options={['Public members', 'Constructors', 'Private members', 'Protected members']}
  answer="Constructors"
/>
<Question
  question="18).Which access specifier allows access only within the class itself?"
  options={['Private', 'Friend', 'Protected', 'Public']}
  answer="Private"
/>
<Question
  question="19).Which keyword is used to inherit a class in C++?"
  options={['extends', 'public/private/protected', 'base', 'inherits']}
  answer="public/private/protected"
/>
<Question
  question="20).What is dynamic polymorphism in C++?"
  options={['Compile-time binding', 'Function overloading', 'Runtime function overriding', 'None of the above']}
  answer="Runtime function overriding"
/>

<div>
<AdBanner />
</div>

<Question
  question="21).Which of the following is true about abstract classes?"
  options={['They cannot be inherited', 'They must contain at least one pure virtual function', 'They can be instantiated', 'They can only contain private members']}
  answer="They must contain at least one pure virtual function"
/>
<Question
  question="22).Which keyword is used to declare a virtual function?"
  options={['virtual', 'override', 'function', 'abstract']}
  answer="virtual"
/>
<Question
  question="23).What is the purpose of a destructor in C++?"
  options={['To delete an object', 'To call other functions', 'To construct an object', 'To allocate memory']}
  answer="To delete an object"
/>
<Question
  question="24).What happens if a base class destructor is not virtual?"
  options={['Memory leak may occur', 'Nothing happens', 'Derived class destructors are always called', 'Code doesn’t compile']}
  answer="Memory leak may occur"
/>
<Question
  question="25).Which class member is shared among all objects of a class?"
  options={['Constructor', 'Private member', 'Static member', 'Public member']}
  answer="Static member"
/>
<Question
  question="26).What is the use of the `this` pointer?"
  options={['Call constructor', 'Access static variables', 'Initialize variables', 'Refer to current object']}
  answer="Refer to current object"
/>
<Question
  question="27).Which keyword prevents a class from being inherited?"
  options={['final', 'sealed', 'const', 'static']}
  answer="final"
/>
<Question
  question="28).Which of these is not a principle of OOP?"
  options={['Recursion', 'Polymorphism', 'Abstraction', 'Encapsulation']}
  answer="Recursion"
/>
<Question
  question="29).Which of the following is true about friend functions?"
  options={['They violate encapsulation', 'They are constructors', 'None of the above', 'They are public']}
  answer="They violate encapsulation"
/>
<Question
  question="30).Which keyword is used to prevent overriding of a method?"
  options={['const', 'final', 'static', 'private']}
  answer="final"
/>

<div>
<AdBanner />
</div>

<Question
  question="31).Can a constructor be virtual?"
  options={['No', 'Yes', 'Only static constructors can be virtual', 'Only if defined in derived class']}
  answer="No"
/>
<Question
  question="32).What is a pure virtual function?"
  options={['A function with default implementation', 'A function that has no definition', 'None of the above', 'A static member function']}
  answer="A function that has no definition"
/>
<Question
  question="33).Which of the following best defines abstraction?"
  options={['Binding data', 'None of the above', 'Inheriting from a base class', 'Hiding implementation details and showing only functionality']}
  answer="Hiding implementation details and showing only functionality"
/>
<Question
  question="34).What is multiple inheritance?"
  options={['A class inheriting multiple classes', 'None of the above', 'A class inheriting from one class', 'Multiple classes inheriting one class']}
  answer="A class inheriting multiple classes"
/>
<Question
  question="35).What is hierarchical inheritance?"
  options={['Derived class inherits grandparent class', 'Multiple classes inherit from one class', 'None of the above', 'One class inherits multiple classes']}
  answer="Multiple classes inherit from one class"
/>
<Question
  question="36).Which of these is a type of polymorphism?"
  options={['Inheritance', 'Operator overloading', 'Constructors', 'Data hiding']}
  answer="Operator overloading"
/>
<Question
  question="37).Which keyword is used to define a friend function?"
  options={['private', 'protected', 'extern', 'friend']}
  answer="friend"
/>
<Question
  question="38).Can a friend function access private members of a class?"
  options={['Only static members', 'Only public members', 'No', 'Yes']}
  answer="Yes"
/>
<Question
  question="39).Which of the following is not true about classes?"
  options={['They support polymorphism', 'They allow abstraction', 'They don’t support inheritance', 'They support data hiding']}
  answer="They don’t support inheritance"
/>
<Question
  question="40).What is the correct order of constructor and destructor calls in inheritance?"
  options={['Base constructor → Derived constructor → Derived destructor → Base destructor', 'Base constructor → Derived constructor → Base destructor → Derived destructor', 'Derived constructor → Base constructor → Derived destructor → Base destructor', 'Derived constructor → Derived destructor → Base constructor → Base destructor']}
  answer="Base constructor → Derived constructor → Derived destructor → Base destructor"
/>

<div>
<AdBanner />
</div>

<Question
  question="41).Which of the following supports function overriding?"
  options={['Virtual functions', 'Static functions', 'Inline functions', 'Friend functions']}
  answer="Virtual functions"
/>
<Question
  question="42).What is hybrid inheritance?"
  options={['Using multiple base classes only', 'Combination of multiple and multilevel inheritance', 'Hierarchical inheritance', 'None of the above']}
  answer="Combination of multiple and multilevel inheritance"
/>
<Question
  question="43).How is runtime polymorphism achieved in C++?"
  options={['Default arguments', 'Virtual functions and pointers', 'Static functions', 'Function overloading']}
  answer="Virtual functions and pointers"
/>
<Question
  question="44).Can static member functions access non-static members?"
  options={['Only protected ones', 'Yes', 'Only in derived class', 'No']}
  answer="No"
/>
<Question
  question="45).Which of the following is an advantage of OOP?"
  options={['Increased redundancy', 'Slower programs', 'Better modularity', 'None of the above']}
  answer="Better modularity"
/>
<Question
  question="46).What is meant by data hiding?"
  options={['Encrypting data', 'Storing data in files', 'Storing data in memory', 'Hiding data from unauthorized access']}
  answer="Hiding data from unauthorized access"
/>
<Question
  question="47).Can private members of a class be accessed directly by objects?"
  options={['Only in structs', 'Only in derived classes', 'No', 'Yes']}
  answer="No"
/>
<Question
  question="48).Which of the following is true about method overloading?"
  options={['It occurs during compile-time', 'None of the above', 'It’s same as overriding', 'It occurs during runtime']}
  answer="It occurs during compile-time"
/>
<Question
  question="49).Which of the following members are inherited by derived class?"
  options={['Protected members', 'Constructors', 'Private members', 'Destructors']}
  answer="Protected members"
/>
<Question
  question="50).Which of these can be overloaded in C++?"
  options={['Constructors', 'All of the above', 'Operators', 'Functions']}
  answer="All of the above"
/>

<div>
<AdBanner />
</div>