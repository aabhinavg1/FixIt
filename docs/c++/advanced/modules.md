...
description: "Modules replace much of textual header inclusion with named interfaces, reducing repeated parsing and macro leakage. Learn the compiler model, practical trade-offs, and production guidance."
tags:
  - C++
  - Modern C++
keywords:
  - module interfaces and build isolation
  - Modules in C++20
sidebar_label: Modules in C++20
---
...
Modules replace much of textual header inclusion with named interfaces, reducing repeated parsing and macro leakage. It also gives a decision framework for choosing the feature instead of treating syntax as the design.
...
The compiler produces a compiled module interface and imports its declarations without reparsing the same header text in every translation unit.

Macros do not cross module boundaries in the same way as declarations, which improves isolation but complicates mixed header/module builds.

Adopt modules at project boundaries first, keep a clear ownership of exported APIs, and verify compiler and build-system support.
